...
export let fillStars = (response) => {
    return response == 'correct' ? checkExisting('correct-scores', 'yellow') : checkExisting('wrong-scores', 'red');
}
...
let checkExisting = (whichSide, color) => {
    let count = 0;
    $all(`.${whichSide} .star--red`).forEach(node=> {
